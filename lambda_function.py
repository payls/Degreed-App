...
def get_access_token(app_settings, token_baseurl):
    ...
    # getting token
    response = requests.post(token_baseurl + '/oauth/token', headers=login_headers, data=payload)
...
def subtract_common_skills(skills_set1, skills_set2):
    ...
    uncommon_skills = []
    final_list = list(
        set([s.get('name') for s in skills_set2]) - set([s.get('name') for s in skills_set1]))
    ...
    if term is not None and term != '':
        ...
        if candidate_skills is not None and candidate_skills != '':
            ...
            if 'position_skills' in candidate_skills:
                ...
                uncommon_skills = subtract_common_skills(profile_skills, position_skills)
                ...
                uncommon_skills = subtract_common_skills(profile_skills, required_skills)
                str = skill_function_name(str, uncommon_skills)
            else:
...
def get_recommend_skills(baseurl, headers, course):
    ...
    ans = requests.get(baseurl + '/api/v2/content/' + course_id + '/skills', headers=headers)
...
def recommend_concurrent_loop(baseurl, headers, recommended_num, recommended_courses):
    ...
    modified_course = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=int(recommended_num)) as executor:
        futures = [executor.submit(get_recommend_skills, baseurl, headers, course) for course in
                   recommended_courses]
...
def matching_schema_concurrent_loop(baseurl, headers, language, filtered_matched_course_list,
                                    recommended_num):
    """Optimization: Using ThreadPool method for getting detailed schema"""
    matching_schema_start_time = time.perf_counter()
    matched_course_list = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=int(recommended_num)) as executor:
        futures = [executor.submit(matching_schema, baseurl, headers, course, language) for course
                   in
...
def recommended_course_list(baseurl, candidate_id, headers, language, request_data, app_settings):
    ...
    recommended_response = requests.get(
        baseurl + '/api/v2/users/' + candidate_id + '/required-learning', headers=headers)
    ...
    if recommended_courses:
        if trigger_source == 'ch_search':
            ...
            modified_course = recommend_concurrent_loop(baseurl, headers, recommended_num,
                                                        recommended_courses)
            ...
            for course in modified_course:
                if course['skills_status_code'] == 200:
                    ...
                    skill_matched = get_search_string(request_data, check_with_fq_skills, skills)
        ...
        matched_course_list = matching_schema_concurrent_loop(baseurl, headers, language,
                                                              filtered_matched_course_list,
                                                              recommended_num)
    ...
    print(f"Time taken for fetching recommended list: {end_time - start_time:0.4f} seconds")
    return result
...
def get_search_course_list(baseurl, headers, language, request_data):
    ...
    url_without_nextBatch = baseurl + 'api/v2/content/?filter[term]=' + filter_search_term
    ...
    end_time = time.perf_counter()
    print(f"Time taken for fetching content filter list: {end_time - start_time:0.4f} seconds")
    return result
...
def get_user_id(email, baseurl, request_headers):
    ...
    if email != '' and email is not None:
        candidate_response = requests.get(baseurl + 'api/v2/users/' + email + '?identifier=email',
...
def time_epoch(date):
    ...
    if date:
        report_completion_date_time = datetime.strptime(date.split('.')[0], '%Y-%m-%dT%H:%M:%S')
...
def get_date(date):
    ...
    if date:
        course_created_date_time = datetime.strptime(date.split('.')[0], '%Y-%m-%dT%H:%M:%S')
...
def get_email(request_data, app_settings):
    ...
    if 'employee_email' in request_data and employee_email != '' and employee_email is not None:
        email = degreed_test_email if use_test_email else employee_email
    elif 'email' in request_data and candidate_email != '' and candidate_email is not None:
        email = degreed_test_email if use_test_email else candidate_email
    elif 'current_user_email' in request_data and current_user_email != '' and current_user_email is not None:
...
def recommendation_trigger_limitations(request_data):
    ...
    if (
            term is not None and term != '') or trigger_source == 'ch_jobs' or trigger_source == 'ch_projects':
        return False
...
def get_combined_concurrent(request_data, app_settings, baseurl, candidate_id, request_headers,
                            language):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        if recommendation_trigger_limitations(request_data):
            futures.append(
                executor.submit(recommended_course_list, baseurl, candidate_id, request_headers,
                                language,
                                request_data, app_settings))
        futures.append(
            executor.submit(get_search_course_list, baseurl, request_headers, language,
                            request_data))
        for future in concurrent.futures.as_completed(futures):
...
def recommended_course_cursor_list(course_len, endlimit, course_limit, offset, next_batch,
                                   recomm_list, con_list):
...
def get_combined_courses(request_data, app_settings):
    ...
    recommended_course = []
    get_combined_list = get_combined_concurrent(request_data, app_settings, baseurl,
                                                candidate_id,
                                                request_headers, language)
    ...
    recommended_list_cursor = recommended_course_cursor_list(course_len, endlimit, course_limit,
                                                             offset,
                                                             next_batch, recomm_list, con_list)
...
def careerhub_entity_search_results_handler(event):
        ...
        for course in recommended_course:
            if course != '':
                ...
                if provider is None and course_attr.get('is-internal') is True:
                    provider = 'Internal'
                course_duration = get_duration(course_attr.get('duration-type'),
                ...
                item = {
                    ...
                    "fields": [
                        ...
                        {
                            "name": "Owner",
                            "value": course_attr.get('owner', '')
                        },
                        {
                            "name": "Published Date",
                            "value": course_created_at
                        }
                    ],
                    "cta_label": cta_label,
                    "tags": "",
    ...
    print(f"Time taken for fetching Handler: {end_handler_time - start_handler_time:0.4f} seconds")
...
def careerhub_homepage_recommended_courses_handler(event):
...
def careerhub_get_entity_details_handler(event):
    ...
    course_details_response = requests.get(baseurl + '/api/v2/content/' + str(entity_id),
                                           headers=request_headers)
    ...
    cta_url = """<div style="text-align:center;padding:20px 10px;"><a target="_blank" style="background-color:#1571ac;padding:10px 20px;display:inline-block;border-radius:4px;color:#fff;text-decoration:none;" href='""" + degreedUrl + """'>View Course on Degreed</a></div> """
...
def careerhub_profile_course_attendance_handler(event):
    ...
    course_attendance_response = requests.get(
        baseurl + '/api/v2/users/' + candidate_id + '/completions', headers=request_headers)
...
def app_handler(event):
    """Trigger name app handler function"""
    request_data = event.get('request_data', {})
    ...
    try:
        if trigger_name == 'careerhub_entity_search_results':
            return careerhub_entity_search_results_handler(event)
        elif trigger_name == 'careerhub_get_entity_details':
            return careerhub_get_entity_details_handler(event)
        elif trigger_name == 'career_planner_recommended_courses':
            return careerhub_homepage_recommended_courses_handler(event)
        elif trigger_name == 'careerhub_profile_course_attendance':
            return careerhub_profile_course_attendance_handler(event)
        ...
        data['error'] = error_str
        error_dict = {'error_str': error_str}
