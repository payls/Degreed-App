...
import json
from fake_data import recommended_course, search_courses, course_list_data, inputs, token_string, entity_details
...
"""
Given in fq profile skills and skill goal empty
test that a response returing by this trigger function is correctly as expected
"""
...
def test_careerhub_entity_search_results_valid_fq():
    ...
    with requests_mock.Mocker() as mock:
        ...
        if(filter_search_term == ''):
            # get course api call with filter query
            url_without_nextBatch = (
                baseurl + "api/v2/content/" + filter_search_term
            )
